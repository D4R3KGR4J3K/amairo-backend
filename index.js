...
app.get('/article/', (req, res) => {
  if(req.query.search !== undefined) {
    if(req.query.search === ''){
      modelArticles.find({}, {_id: 0, __v:0}).limit(10).then(query => res.send(query));
      return;
    }
    
    modelArticles.find({articleTitle: req.query.search}, {_id: 0, __v:0}).limit(10).then(query => res.send(query));
    return;
  }
...
app.get('/services/', (req, res) => {
  if(req.query.search === undefined || req.query.search === null){
    modelService.find().limit(3).then(items => {
      const list = [];
      items.forEach(item => {
        list.push(item);
      });
